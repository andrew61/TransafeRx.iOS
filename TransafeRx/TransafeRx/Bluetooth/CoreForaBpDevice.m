...
- (void)centralManager:(CBCentralManager *)central didDiscoverPeripheral:(CBPeripheral *)peripheral advertisementData:(NSDictionary *)advertisementData RSSI:(NSNumber *)RSSI
{
    ...
    if (localName != nil && ([localName rangeOfString:self.deviceName].location != NSNotFound || [localName rangeOfString:@"TNG BP"].location != NSNotFound ))
    {
        NSLog(@"FOUND: %@", localName);
        
//        if([[KeychainManager sharedManager]getBPDevice] == nil){
//            [[KeychainManager sharedManager]setBPDevice:localName];
//        }
...
- (void)foraBpClearMemory{
    char const Command[] = {0x51,  0x52,  0x01,    0x00,    0x00,   0x00,  0xA3,   0x47};//(70 dec or 46 hex with 0 for user) 71 dec or 47 hex with 1 for user
    NSData *commandData = [NSData dataWithBytes: Command length:(unsigned)sizeof(Command)];
